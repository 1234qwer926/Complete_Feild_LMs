...
function FileUploadComponent() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://java-application-pulselms.onrender.com/upload', formData, {
        headers: {
  ...
  const handleGenerateUrl = async () => {
    ...
    try {
      // --- START OF UPDATE 2 ---
      // Switched to axios and added withCredentials
      const response = await axios.get(`https://java-application-pulselms.onrender.com/generate-presigned-url/${fileKey}`, {
        withCredentials: true,
      });
      // --- END OF UPDATE 2 ---
